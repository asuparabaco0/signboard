...
//replace article with div(purpose for visibility)
    ...
    >
      ...
      {text?.split(/(https?:\/\/\S+)/g).map((fragment, i) =>
        i % 2 === 0 ? (
          ...
          /*if text contains URL, convert it to Link */
...
const CheckIcon = styled(_CheckIcon)`
  ...
  color: ${color.Green};
`
/*attrs method to set Default attrs
<DeleteButton /> =
...
function useDragAutoLeave(timeout: number = 100) {
  //dragOver below and above one have different scopes.
  ...
  // as const never let us change discription
}
